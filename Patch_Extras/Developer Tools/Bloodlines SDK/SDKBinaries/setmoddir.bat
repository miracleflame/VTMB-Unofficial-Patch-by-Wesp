...
:ProcessModDir
	set ProcDirRan=1
	for %%m in ("Vampire\" "Vampire.exe") do if not exist "%~1\..\%%~m" (
	...
	set DirIsValid=1
	%Sfk% filter "GameCfg.ini" -where "GameExeDir=" -rep "|GameExeDir=*|GameExeDir=%GameRoot%|" -write -yes
	%Sfk% filter "GameCfg.ini" -where "GameExe=" -rep "|GameExe=*|GameExe=%GameRoot%\Vampire.exe|" -write -yes
	%Sfk% filter "GameCfg.ini" -where "BSPDir=" -rep "|BSPDir=*|BSPDir=%ModDir%\maps|" -write -yes
	%Sfk% filter "GameCfg.ini" -where "ModDir=" -rep "|ModDir=*|ModDir=%ModDir%|" -write -yes
exit /b
