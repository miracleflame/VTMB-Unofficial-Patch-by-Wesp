...
__bpydoc__ = """\
Try to show index of vertices selected.

Usage:

Select the mesh you want to work on, enter Edit Mode and select the verts
to identify.  Then run this script from the 3d View's Mesh->Scripts menu.
...
# (c) 2004-2006 c Berthe (loic+blender@lilotux.net)
# released under Blender Artistic License

######################################################################

import Blender
from Blender import NMesh,Object,Material,Texture,Image,Draw,Scene,Window,Mesh
...
def draw():
	...
	Button("Show verts selected DS(2)",EVENT_BEVEL,10,10,160,25)   #x,y,xd,yd

	Button("Exit",EVENT_EXIT,210,10,80,25)
...
def bevent(evt):
	...
	elif evt == EVENT_BEVEL	 : vertsselect()   #bevel()
...
def vertsselect():
	...
	meshname = object.getName()
	mesh = Mesh.Get(meshname)   #object.getData()
	vert_select = mesh.verts.selected()
	for verts in vert_select:
		print "Test: " ,verts
	#result=Blender.Draw.PupMenu(str(vert_select) + '%t|OK' ) #("Not the right file for modifying! %t|OK")
