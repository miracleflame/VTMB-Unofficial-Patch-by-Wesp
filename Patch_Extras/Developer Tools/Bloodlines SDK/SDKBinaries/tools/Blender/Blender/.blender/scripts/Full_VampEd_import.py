...
Blender: 240
Group: 'Import'
Tip: 'Imports model from Bloodlines .X file format'
"""

__author__ = 'Mod by DDLullu'
__version__ = '2.00'
__bpydoc__ = """\
This script IMPORTs .x file created by VampEd 0.92 or PackFile Explorer 3.09
...
1. Run VampEd or PackFile Explorer and extract the desired model as a .x file.

2. Put this script and "VampEd_mdl.py" in the scripts folder of blender. Run Blender, find the Import in file menu.
...
9. For PackFile Explorer don't move the vertices at 0,0,0, they are the lods vertices.
...
# It is now possible to move the models vertices.                                      #
# 1. Run VampEd or PackFile Explorer and extract the desired model as a .x file.       #
# 2. Put this script and "VampEd_mdl.py" in the scripts folder of Blender.             #
#    Run Blender, find the import in file menu.                                        #
# 3. Choose Bloodlines_X in the IMPORT menu and load the model you exported in step 1. #
...
# 8. Select the model to reload and view your changes in VampEd, or run the game.      #
# 9. For PackFile don't move the vertices at 0,0,0, they are the lods vertices.        #
########################################################################################
...
class xImport:
	...
	def writeMaterials(self, nr_mat, idx, mat_list, tex_list):
		...
		if not tex_n:				######################################for packfile explorer
			l = self.lines[nr_mat + 7]    #ve
			fix_3_line = self.CleanLine(l)
			tex_n = fix_3_line.split()
...
arg = __script__['arg']
Blender.Window.FileSelector(my_callback, "Import Bloodlines .X")
