...
for %%p in (SourceDir ShortPath) do if not defined %%~p (
	%Sfk% echo [yellow]- Canceled by user?
	goto quit
)
if not defined SoundsImported if not defined VCDMode (
...
for %%m in ("%TargetPath%\%ShortPath%\*.wav") do if exist "%%~dpnm_work.wav" del /f /q "%%~dpnm_work.wav"> nul
...
) do move /y "%%~m" "%SourceDir%\%ShortPath%\"> nul
...
:DoImport
	...
	if not defined ShortPath for /f "delims=" %%s in ('call %Sfk% echo "%~dp1#" +filter -lerep "|\#||" -rep "|*\sound\||"') do if /i not "%%~s\"=="%~dp1" (set "ShortPath=%%~s") else (set "ShortPath=.")
	if not defined SourceDir for /f "delims=" %%s in ('call %Sfk% echo "%~dp1#" +filter -lerep "|\#||" -rep "|\%ShortPath%||"') do set "SourceDir=%%~s"
	...
	if exist "%~2\%ShortPath%\%~n1.vcd" (
		...
		) do for %%p in ("%SourceDir%" "%ModDir%\sound" "%GameDir%\sound") do (
			rem If found something, import this to Target dir...
			if not exist "%~2\%%~l.wav" for %%f in ("%~2\%%~l.wav") do (
				if exist "%%~p\%%~l.wav" (
