...
for %%m in (Vampire Unofficial_Patch) do (
if /i "%NewModDir%"=="%%~m" (
...
if "%ErrorLevel%"=="6" (
	...
	if exist "%GameExeDir%\Unofficial_Patch\%%~m\" (
		xcopy /s /c /i /r /q /y "%GameExeDir%\Unofficial_Patch\%%~m" "%GameExeDir%\%NewModDir%\%%~m\"> nul
	))
	for /d %%m in ("%GameExeDir%\Unofficial_Patch\sound\*") do (
	if /i not "%%~nxm"=="character" (
		xcopy /s /c /i /r /q /y "%%~m" "%GameExeDir%\%NewModDir%\sound\%%~nxm\"> nul
	))
	for /d %%m in ("%GameExeDir%\Unofficial_Patch\sound\character\*") do (
	if /i not "%%~nxm"=="dlg" (
