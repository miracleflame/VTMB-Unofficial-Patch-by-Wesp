...
for %%f in (
	...
	if not exist "%%~f" (
		color 0c
		echo ERROR! No "%%~nxf" found. Exiting...
		echo>>"..\..\..\sdk_errors.log" [%date% %time%] PrepWizard error: Missing "%%~f" file.
		exit
	)
)
...
if not exist "%VampireDir%\pack?*.vpk" (
	...
	%Sfk% filter "GameCfg.ini" -where "GameDir=" -rep "|GameDir=*|GameDir=%VampireDir%|" -write -yes> nul
...
%UnZip% x %Assets% -o"%TargetDir%\" -y -aoa > nul
