...
for %%z in ("%~dpn1.vtf") do (
	echo   Obtained VTF size: %%~zz bytes.
)
...
for %%z in ("%~dpn1.%ExportFormat%") do (
	echo   Obtained %ExportFormat% size: %%~zz bytes.
)
...
for %%x in ("%~dpn1.tth") do (
for %%z in ("%~dpn1.ttz") do (
	echo   Source TTZ size:  %%~zx/%%~zz bytes.
))
rem Get obtained VTF size...
for %%z in ("%~dpn1.vtf") do (
	echo   Obtained VTF size:   %%~zz bytes.
...
if "%AutoFixVtfVersion%"=="1" (
	...
	for /f %%s in ('call %Sfk% hexdump -offlen 12 1 -pure -nofile "%VtfFile%"') do (
		set "VHdrSz=%%~s"
	)
)
if "%AutoFixVtfVersion%"=="1" (
	for /f %%v in ('call %Sfk% dec %VHdrSz%') do (
		if %%~v GEQ 64 if %%~v LEQ 128 (
			set "VHdrSzDec=%%~v"
...
for %%m in ("%VtfFile%") do (
	set "VtfSize=%%~zm"
	if %%~zm GEQ 66 if %%~zm LEQ 268435456 (
...
rem Get obtained TTZ size...
	for %%z in ("%%~dpnm.ttz") do (
	  set "TtzNumSize=%%~zz"
	  echo   Obtained TTZ size:	%%~zz bytes.
	)
)
...
:ReadTemplates
	...
	if "%UseTemplates%"=="1" if exist %Templates% (
