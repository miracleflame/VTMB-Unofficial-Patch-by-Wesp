...
:paint.net-plugins
 ...
 for %%p in ("%ProgramFiles%" "%ProgramFiles(x86)%" "%ProgramW6432%") do (
 	if exist "%%~p\Paint.NET\" call :InstallPNPlugs "%%~p")
...
:InstallPNPlugs
	if exist "plugins\paintdotnet.7z" if exist "%~1\Paint*Net?*" (
	service\7za.exe x "plugins\paintdotnet.7z" -o"%~1\" -y -aoa> nul
	if exist "%~1\FileTypes\VtfFileType.dll" set PlgInstSuccess=1)
exit /b

:InstallPSPlugs
	if exist "plugins\photoshop.7z" if exist "%~1\Photoshop.exe" (
	service\7za.exe x "plugins\photoshop.7z" -o"%~1\" -y -aoa> nul
